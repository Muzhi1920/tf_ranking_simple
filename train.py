...
TRAIN=tensorflow_ranking/examples/tf_data/train_elwc.tfrecord && \
EVAL=tensorflow_ranking/examples/tf_data/eval_elwc.tfrecord && \
VOCAB=tensorflow_ranking/examples/tf_data/vocab.txt && \
rm -rf $MODEL_DIR && \
bazel build -c opt \
...
from lib.estimator import EstimatorBuilder
from lib.pipeline import RankingPipeline
...
flags.DEFINE_bool(
    ...
    "Whether the inference will be performed with the listwise tf_data format "
...
def scoring_function(context_features, example_features, mode):
    ...
    for layer_width in layer_dims:
        ...
        cur_layer = tf.compat.v1.layers.dropout(inputs=cur_layer, rate=FLAGS.dropout_rate, training=is_training)
