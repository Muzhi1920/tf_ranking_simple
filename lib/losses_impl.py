...
class CircleLoss(_ListwiseLoss):
  ...
  For a model that outputs similarity scores `s` on tf_data point with
  ...
  defined for the binary label, p for tf_data points with positive labels and n for
  tf_data points with negative labels.
...
class ClickEMLoss(_PointwiseLoss):
  ...
  def _compute_latent_prob(self, clicks, exam_logits, rel_logits):
    ...
    Args:
      clicks: A 2-D `Tensor` for clicks as observed tf_data. A value >= 1.0 is
        treated as clicked.
