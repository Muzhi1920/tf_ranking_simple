...
class EstimatorBuilder(object):
    ...
    def _transform_fn(self, features, mode):
        ...
        ## point-wise and pair-wise inference here
        if (mode == tf_estimator.ModeKeys.PREDICT and not self._hparams.get("listwise_inference")):
    ...
    def _model_fn(self):
        ...
        ranking_head = head.create_ranking_head(
            loss_fn=losses.make_loss_fn(self._hparams.get("loss"),
...
def make_dnn_ranking_estimator(
    ...
    Args:
      ...
      listwise_inference: (bool) Whether the inference will be performed with the
        listwise tf_data format such as `ExampleListWithContext`.
