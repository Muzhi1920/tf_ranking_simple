...
class RankingPipeline(object):
    ...
    Note that you may:
      ...
      * pass `dataset_reader` for reading different `tf.Dataset`s. We recommend
        using TFRecord files and storing your tf_data in `tfr.tf_data.ELWC` format.
    ...
    class CustomizedDatasetRankingPipeline(tfr.ext.pipeline.RankingPipeline):
      def _make_dataset(self,
                        ...
                        num_epochs=None):
        # Creates your own dataset, plese follow `tfr.tf_data.build_ranking_dataset`.
    ...
    def _make_dataset(self,
        ...
        Args:
          ...
          input_pattern: (str) File pattern for the input tf_data.
          ...
          num_epochs: (int) The number of times the input dataset must be repeated.
            None to repeat the tf_data indefinitely.

        Returns:
          A tuple of (feature tensors, label tensor).
        """

        label_column = tf.feature_column.numeric_column(self._label_feature_name, dtype=self._label_feature_type, default_value=_PADDING_LABEL)

        context_feature_spec = tf.feature_column.make_parse_example_spec(self._context_feature_columns.values())
        example_feature_spec = tf.feature_column.make_parse_example_spec(list(self._example_feature_columns.values()) + [label_column])
    ...
    def _export_strategies(self, event_file_pattern, assets_extra=None):
        ...
        def _compare_fn(best_eval_result, current_eval_result):
            ...
            is_current_the_best = (self._best_exporter_metric_higher_better == (
    ...
    def _train_eval_specs(self):
        ...
        eval_spec = tf_estimator.EvalSpec(
            ...
            exporters=self._export_strategies(event_file_pattern="eval_on_eval/*.tfevents.*", assets_extra=self._hparams.get("assets_extra")),
            throttle_secs=5)
    ...
    def train_and_eval(self, local_training=True):
        ...
        train_spec, eval_spec = self._train_eval_specs()
