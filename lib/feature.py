...
def encode_listwise_features(features,
    ...
    if context_feature_columns:
        ...
        context_features = {
            name: context_cols_to_tensors[col] for name, col in six.iteritems(context_feature_columns)
        }
    ...
    if example_feature_columns:
        if feature_column_lib.is_feature_column_v2(example_feature_columns.values()):
            example_specs = tf.compat.v2.feature_column.make_parse_example_spec(example_feature_columns.values())
        else:
            example_specs = tf.compat.v1.feature_column.make_parse_example_spec(example_feature_columns.values())
        ...
        # Reshape [batch_size, input_size] to [batch * input_size] so that features are encoded.
        reshaped_features = {}
        for name in example_specs:
            ...
            try:
                ...
                raise ValueError(
                    "2nd dimension of tensor must be equal to input size: {}, but found feature {} with shape {}.".format(
        ...
        example_cols_to_tensors = encode_features(
            ...
            scope=scope)
        example_features = {
            name: utils.reshape_first_ndims(example_cols_to_tensors[col], 1, [batch_size, input_size])
...
def encode_pointwise_features(features,
    ...
    context_features = {}
    if context_feature_columns:
        context_cols_to_tensors = encode_features(features, context_feature_columns.values(), mode=mode, scope=scope)
        context_features = {
            name: context_cols_to_tensors[col] for name, col in six.iteritems(context_feature_columns)
        }

    example_features = {}
    if example_feature_columns:  # Handles the case when tf.Example is used as input during serving.
        example_cols_to_tensors = encode_features(features, example_feature_columns.values(), mode=mode, scope=scope)
        example_features = {
            name: tf.expand_dims(example_cols_to_tensors[col], 1) for name, col in six.iteritems(example_feature_columns)
        }
    return context_features, example_features
