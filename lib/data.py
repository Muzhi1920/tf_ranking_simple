...
"""Input tf_data parsing for tf-ranking library.

Supports the following tf_data formats:
...
def _get_descriptor_set():
    ...
    file_proto = proto.file.add(name=_FILE_NAME, package=_PACKAGE, syntax="proto3")
    message_proto = file_proto.message_type.add(name=_MESSAGE_NAME)
    message_proto.field.add(name=_EXAMPLES_FIELD_NAME, number=1, type=pb.FieldDescriptorProto.TYPE_BYTES,
                            label=pb.FieldDescriptorProto.LABEL_REPEATED)
    message_proto.field.add(name=_CONTEXT_FIELD_NAME, number=2, type=pb.FieldDescriptorProto.TYPE_BYTES)
    return proto
...
def _decode_as_serialized_example_list(serialized):
    ...
    sizes, (serialized_context, serialized_list) = tf.io.decode_proto(
        ...
        descriptor_source=(b"bytes://" + _get_descriptor_set().SerializeToString()))
...
class _RankingDataParser(object):
    """Interface for ranking tf_data parser."""
    ...
    def __init__(self,
        ...
        if not example_feature_spec:
            raise ValueError("example_feature_spec {} must not be empty.".format(example_feature_spec))
...
class _ExampleInExampleParser(_RankingDataParser):
    ...
    def _decode_as_serialized_example_list(self, serialized):
        ...
        mask = tf.scatter_nd(indices=lists.indices, updates=tf.ones_like(lists.values, dtype=tf.int32), shape=lists.dense_shape)
        sizes = tf.reduce_sum(input_tensor=mask, axis=1)
        return features["serialized_context"], tf.sparse.to_dense(lists, default_value=""), sizes

    def parse(self, serialized):
        """See `_RankingDataParser`."""
        (serialized_context, serialized_list, sizes) = self._decode_as_serialized_example_list(serialized)

        # Use static batch size whenever possible.
        batch_size = serialized_context.get_shape().as_list()[0] or tf.shape(input=serialized_list)[0]
        ...
        if list_size:
            ...
            def pad_fn():
                return tf.pad(tensor=serialized_list, paddings=[[0, 0], [0, list_size - cur_list_size]], constant_values="")

            serialized_list = tf.cond(pred=cur_list_size > list_size, true_fn=truncate_fn, false_fn=pad_fn)
            cur_list_size = list_size

        features = {}
        example_features = tf.compat.v1.io.parse_example(tf.reshape(serialized_list, [-1]), self._example_feature_spec)
        ...
        if self._context_feature_spec:
            features.update(tf.compat.v1.io.parse_example(tf.reshape(serialized_context, [batch_size]), self._context_feature_spec))
    ...
    def parse(self, serialized):
        ...
        for k, s in six.iteritems(example_feature_spec):
            if not isinstance(s, tf.io.FixedLenFeature):
                continue
            fixed_len_sequence_features[k] = tf.io.FixedLenSequenceFeature(s.shape, s.dtype, allow_missing=True)
        ...
        context, examples, sizes = tf.io.parse_sequence_example(serialized, context_features=context_feature_spec, sequence_features=sequence_features)
        ...
        if not sizes:
            for name, example in examples.items():
                if isinstance(example, tf.RaggedTensor):
                    ...
                    sizes[name] = 1 + tf.math.segment_max(example.indices[:, 1], example.indices[:, 0])
        ...
        for k, v in six.iteritems(non_trivial_padding_values):
            ...
            rank = tf.reshape(tf.tile(tf.range(tf.shape(input=tensor)[1]), [tf.shape(input=tensor)[0]]), tf.shape(input=tensor))
            tensor = tf.compat.v1.where(tf.less(rank, tf.cast(size, tf.int32)), tensor, tf.fill(tf.shape(input=tensor), tf.cast(v, tensor.dtype)))
            examples[k] = tensor

        list_size_arg = list_size
        if list_size is None:
            # Use dynamic list_size. This is needed to pad missing feature_list.
            list_size_dynamic = tf.reduce_max(input_tensor=tf.stack([_bounding_shape(t)[1] for t in six.itervalues(examples)]))
        ...
        for k, t in six.iteritems(examples):
            ...
            def truncate_fn(t=t, ndims=ndims, new_shape=new_shape):
                ...
                if isinstance(t, tf.sparse.SparseTensor):
                    return tf.sparse.slice(t, [0] * ndims, tf.cast(new_shape, dtype=tf.int64))
                elif isinstance(t, tf.RaggedTensor):
                    return t[:, :new_shape[1], ...]
                else:
                    return tf.slice(t, [0] * ndims, new_shape)

            def pad_fn(k=k, t=t, ndims=ndims, num_frames=num_frames, new_shape=new_shape):
                """Pads the tensor."""
                if isinstance(t, tf.sparse.SparseTensor):
                    ...
                    flattened_shape = tf.concat([[new_shape[0] * list_size], new_shape[2:]], axis=0)
                    flattened_sparse_tensor = tf.sparse.reshape(sparse_tensor, flattened_shape)
                    # Convert to a ragged tensor of shape [batch_size * list_size, ...].
                    ragged_tensor = tf.RaggedTensor.from_sparse(flattened_sparse_tensor, row_splits_dtype=t.row_splits.dtype)
                    # Reshape ragged tensor to [batch_size, list_size, ...]
                    return tf.RaggedTensor.from_uniform_row_length(ragged_tensor, list_size)
                else:
                    # Paddings has shape [n, 2] where n is the rank of the tensor.
                    paddings = tf.stack([[0, 0], [0, list_size - num_frames]] + [[0, 0]] * (ndims - 2))
                    return tf.pad(tensor=t, paddings=paddings, constant_values=padding_values[k])

            tensor = tf.cond(pred=num_frames > list_size, true_fn=truncate_fn, false_fn=pad_fn)
...
def make_parsing_fn(data_format,
                    ...
                    shuffle_examples=False,
                    seed=None):
    """Returns a parsing fn for a standard tf_data format.
    ...
    Returns:
      ...
      is a string Tensor representing the serialized tf_data in the specified
...
def build_ranking_dataset_with_parsing_fn(
    ...
    Args:
      file_pattern: (str | list(str)) List of files or patterns of file paths
        containing serialized tf_data. See `tf.gfile.Glob` for pattern rules.
      parsing_fn: (function) It has a single argument parsing_fn(serialized).
        Users can customize this for their own tf_data formats.
    ...
    if reader_num_threads == tf.data.experimental.AUTOTUNE:
        dataset = dataset.interleave(lambda filename: reader(filename, *(reader_args or [])), num_parallel_calls=reader_num_threads)
    else:
        # cycle_length needs to be set when reader_num_threads is not AUTOTUNE.
        dataset = dataset.interleave(lambda filename: reader(filename, *(reader_args or [])), cycle_length=reader_num_threads, num_parallel_calls=reader_num_threads)
    ...
    # Prefetching allows for tf_data fetching to happen on host while model runs
    # on the accelerator. When run on CPU, makes tf_data fetching asynchronous.
...
def build_ranking_dataset(file_pattern,
                          ...
                          seed=None,
                          **kwargs):
    """Builds a ranking tf.dataset with a standard tf_data format.
...
def build_ranking_serving_input_receiver_fn(data_format,
    ...
    """Returns a serving input receiver fn for a standard tf_data format.
